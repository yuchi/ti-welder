...
ImageBuilder.prototype.place = function (src, config) {
	...
	if (_.isArray(src)) {
		return this.placeAny(src, config);
	}

	var easy = config.easy;
...
ImageBuilder.prototype.placeAny = function(srcs, config) {
	var easy = config.easy;

	config.easy = true;

	var found = _.find(srcs, function (src) {
		return this.place(src, config);
	}, this);

	config.easy = easy;

	if (!easy && !found) {
		throw new Error("No image found for " + srcs.join(" or "));
	}
